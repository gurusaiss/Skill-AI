...
// Auto-generate and assign a role-specific pre-assessment for a newly created employee.
// Runs in setImmediate (non-blocking). Stores a GeneratedContent record for admin review.
async function autoAssignPreAssessment(user) {
  ...
  try {
    ...
    const resolvedJD     = user.jobDescription || roleMatch?.jobDescription || '';
    const resolvedSkills = (user.jdSkills?.length ? user.jdSkills : null) || roleMatch?.skills || [];
    ...
    console.log(`[auto-assessment] Pre-assessment assigned to ${user.email} (${user.jobRole})`);
  } catch (e) {
